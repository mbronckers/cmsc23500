...
public class JoinOptimizer {
    ...
    public static OpIterator instantiateJoin(LogicalJoinNode lj,
        ...
        if (lj.p != Predicate.Op.EQUALS) { // Hash joins don't support equality so can only do nested loop
            j = new Join(p,plan1,plan2);
        }
        else {
            // To determine which join to use, we have a simple heuristic of if a table fits in memory
            // then it's better to use a hash join, otherwise use simple nested loop join
            int bpSize = Database.getBufferPool().getNumPages();
            int tb1Size = ((HeapFile) Database.getCatalog().getDatabaseFile(t1id)).numPages();
            int tb2Size = ((HeapFile) Database.getCatalog().getDatabaseFile(t2id)).numPages();

            if (tb1Size + tb2Size <= bpSize) {
                j = new GraceHashJoin(p, plan1, plan2);
            }
            else if (tb1Size <= bpSize && tb2Size <= bpSize) {
                if (tb1Size <= tb2Size) {
                    j = new HashEquiJoin(p, plan2, plan1);
                }
                else {
                    j = new HashEquiJoin(p, plan1, plan2);
                }
            }
            else if (tb1Size <= bpSize) {
                j = new HashEquiJoin(p, plan1, plan2);
            }
            else if (tb2Size <= bpSize) {
                j = new HashEquiJoin(p, plan2, plan1);
            }
            else {
                j = new Join(p, plan1, plan2);
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            if (j.p != Predicate.Op.EQUALS) { // Hash joins don't support equality so can only do nested loop
                return cost1 + card1 * cost2 + card1 * card2;
            }
            else {
                // variables to do approximations
                int t1id = Database.getCatalog().getTableId(j.t1Alias);
                int t2id = Database.getCatalog().getTableId(j.t2Alias);
                int bpSize = Database.getBufferPool().getNumPages();
                int tb1Size = ((HeapFile) Database.getCatalog().getDatabaseFile(t1id)).numPages();
                int tb2Size = ((HeapFile) Database.getCatalog().getDatabaseFile(t2id)).numPages();
    
                if (tb1Size <= bpSize || tb2Size <= bpSize) { // GraceHashJoin and HashEquiJoin cost approx
                    return cost1 + cost2 + card1 * card2; 
                }
                else { // Join cost approx
                    return cost1 + card1 * cost2 + card1 * card2;
                }
            }
        }
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if (joinOp != Predicate.Op.EQUALS) {
            ...
            /* Extra credit 1b: assume each value in smaller table has matching
               value in larger table. */
            TableStats t1_stats = stats.get(table1Alias);
            TableStats t2_stats = stats.get(table2Alias);
            
            if (t1_stats != null && t2_stats != null ) {

                /* Get distinct values from each (t1,column1) & (t2,column2) */
                int num_distinct_t1 = t1_stats.distinctTuples(field1PureName);
                int num_distinct_t2 = t2_stats.distinctTuples(field2PureName);

                /* Error handling */
                if (num_distinct_t1 != -1 && num_distinct_t2 != -1) {

                    /* Follow formula given in extra credit 1b */
                    double join_selectivity = (double) 1.0/((double) Math.max(num_distinct_t1, num_distinct_t2));
                    
                    /* Logging */
                    System.out.println("selectivity:"+join_selectivity);
                    System.out.println("card1:"+card1);
                    System.out.println("card2:"+card2);

                    int table_join_cardinality = (int) ((double) card1 * (double) card2 * join_selectivity);
                    
                    System.out.println("cardinality:" + table_join_cardinality);            
                    if (table_join_cardinality < Math.max(card1, card2)) return table_join_cardinality;
     ...
     */
    @SuppressWarnings("unchecked")
    public Set<BitSet> enumerateSubsetsBit(Vector<LogicalJoinNode> v, int size) {
        Set<BitSet> els = new HashSet<BitSet>();
        els.add(new BitSet(v.size()));
        // Iterator<Set> it;
        // long start = System.currentTimeMillis();
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        int r = enumerateSubsetsBit(joins, 1).size();

        for (int i = 1; i <= r; i++) {
            Set<BitSet> sbs = enumerateSubsetsBit(joins, i);

            for (BitSet bs : sbs) {
                Set<LogicalJoinNode> temp = new HashSet<LogicalJoinNode>();
                for (int j = 0; j < joins.size(); j++) {
                    if (bs.get(j) == true) {
                        temp.add(joins.get(j));
                    }
                }
                
                CostCard top = new CostCard();
                top.cost = Double.MAX_VALUE;
                
                for (LogicalJoinNode j : temp) {
                    CostCard c = computeCostAndCardOfSubplan(stats, filterSelectivities, j, temp, top.cost, pc);
                    if (c != null) {
                        if (c.cost < top.cost) {
                            top = c;
                        }
                ...
                pc.addPlan(temp, top.cost, top.card, top.plan);
            }
        }
        ...
        Vector<LogicalJoinNode> order = pc.getOrder(new HashSet<LogicalJoinNode>(joins));
